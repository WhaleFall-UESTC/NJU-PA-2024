...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, cpu.mie = cpu.mpie, cpu.mpie = 1, s->dnpc = GCSR(0) + 4);  // 0x341 is mepc

  // From RISC-V-READER Page27
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc, s->dnpc = s->pc + imm, ft(s)); 
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc, s->dnpc = (src1 + imm) & 0xfffffffe, ft(s));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, do { word_t u = Mr(src1 + imm, 2); R(rd) = SEXT_H(u);} while(0));
