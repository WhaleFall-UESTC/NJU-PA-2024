...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, if (src1 + imm == 0x80012fbc) {printf("%x: %x %x %x %x\n", s->pc, src1, src2, imm, src1 + imm);} Mw(src1 + imm, 1, src2));
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, ECALL(s->dnpc)); 
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = GCSR(0) + 4);  // 0x341 is mepc
